...
This action is available only for eligible non-Gmail `Email/Password` users.
...
For non-Gmail credential users created from the mobile app:
...
The button appears only for non-Gmail `Email/Password` users.
...
- `temp/ec2/user-guide.html` for the mobile app user guide
